...
void initSodY(Array Q, int i, int j, const DeviceParams &params)
{
  ...
  void initProfile(Array Q, int i, int j, const DeviceParams &params, const RandomPool &random_pool) {
    ...
    Q(j, i, IU) = params.profile.compute_from_spline(y, Profile::IU);
    Q(j, i, IV) = params.profile.compute_from_spline(y, Profile::IV);
    Q(j, i, IP) = params.profile.compute_from_spline(y, Profile::IP) * (1.0 + pert);
  }
}
