...
real_t fastMagnetoAcousticSpeed(const State &q, const DeviceParams &params, IDir idir)
{
  ...
  return Kokkos::sqrt(0.5 * (a2 + B2 + Kokkos::sqrt((a2 + B2) * (a2 + B2) - 4.0 * a2 * bi * bi)));
}
...
real_t ComputeGlobalDivergenceSpeed(Array Q, const Params &full_params)
{
  ...
  Kokkos::parallel_reduce(
      "Compute Global Divergece Speed",
      ...
      KOKKOS_LAMBDA(int i, int j, real_t &u_max, real_t &lambda_max) {
        ...
        real_t lambda_x   = Kokkos::max(Kokkos::abs(q[IU] - fastMagnetoAcousticSpeed(q, params, IX)),
                                      Kokkos::abs(q[IU] + fastMagnetoAcousticSpeed(q, params, IX)));
        real_t lambda_y   = Kokkos::max(Kokkos::abs(q[IV] - fastMagnetoAcousticSpeed(q, params, IY)),
                                      Kokkos::abs(q[IV] + fastMagnetoAcousticSpeed(q, params, IY)));
...
KOKKOS_INLINE_FUNCTION
State operator*(real_t q, const State &a) { return a * q; }
...
State getConsJumpState(const State &qL, const State &qR, const DeviceParams &params)
{
  ...
  real_t E              = qJump[IR] / (2.0 * (params.gamma0 - 1.0) * betaLn) + 0.5 * u2bar * qJump[IR] +
             0.5 * (qL[IR] + qR[IR]) * (qMixed[IU] + qMixed[IV] + qMixed[IW]) -
             0.25 * (qL[IR] + qR[IR]) / ((params.gamma0 - 1.0) * beta2bar) * betaJump + qMixed[IBX] + qMixed[IBY] +
             qMixed[IBZ] + qMixed[IPSI];
  return {qJump[IR],
