...
public:
  ...
  ThermalConductionFunctor(const Params &full_params) : full_params(full_params) {};
        ...
        KOKKOS_LAMBDA(const int i, const int j) {
          ...
          if (j == params.jbeg && params.bctc_ymin != BCTC_NONE)
          {
            ...
            case BCTC_FIXED_TEMPERATURE:
              FU = kappaU * 2.0 * (TC - params.bctc_ymin_value) / dy;
              break;
            case BCTC_FIXED_GRADIENT:
              FU = kappaU * params.bctc_ymin_value;
              break;
            case BCTC_NO_CONDUCTION:
              FU = FD;
              break;
            case BCTC_NO_FLUX:
              FU = 0.0;
          ...
          if (j == params.jend - 1 && params.bctc_ymax != BCTC_NONE)
          {
            ...
            {
            case BCTC_FIXED_TEMPERATURE:
              FD = kappaD * 2.0 * (params.bctc_ymax_value - TC) / dy;
              break;
            case BCTC_FIXED_GRADIENT:
              FD = kappaD * params.bctc_ymax_value;
              break;
            case BCTC_NO_CONDUCTION:
              FD = FU;
              break;
