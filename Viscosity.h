...
public:
  ...
  ViscosityFunctor(const Params &full_params) : full_params(full_params) {};
          ...
          auto computeViscousFlux = [&](IDir dir)
          {
            ...
            for (int side = 1; side < 3; ++side)
            {
              ...
              if (dir == IX)
              {
                ...
                real_t dudy =
                    0.25 * one_over_dy *
                    (stencil[2][side][IU] - stencil[0][side][IU] + stencil[2][side - 1][IU] - stencil[0][side - 1][IU]);
                real_t dvdy =
                    0.25 * one_over_dy *
                    (stencil[2][side][IV] - stencil[0][side][IV] + stencil[2][side - 1][IV] - stencil[0][side - 1][IV]);
              ...
              else if (dir == IY)
              {
                ...
                real_t dudx =
                    0.25 * one_over_dx *
                    (stencil[side][2][IU] - stencil[side][0][IU] + stencil[side - 1][2][IU] - stencil[side - 1][0][IU]);
                real_t dvdx =
                    0.25 * one_over_dx *
                    (stencil[side][2][IV] - stencil[side][0][IV] + stencil[side - 1][2][IV] - stencil[side - 1][0][IV]);
