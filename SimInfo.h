...
{
  Reader() = default;
  Reader(const std::string &filename) : reader(filename) {};
  ~Reader() = default;
  ...
  template <typename T>
  auto GetMapValue(const std::map<std::string, T> &map, const std::string &section, const std::string &name,
      ...
      */
      ...
      for (auto p_var : map_section)
      {
        ...
        o << std::setw(std::max(var_name.length(), name_width)) << var_name << " = "
          << std::setw(std::max(val.value.length(), value_width)) << val.value << (val.from_file ? "" : " ; default ")
          << std::endl;
      }
    }
...
struct DeviceParams
{
  ...
  void init_from_inifile(Reader &reader)
  {
    ...
    std::map<std::string, RiemannSolver> riemann_map{{"hll", HLL},
                                                     {"hllc", HLLC},
                                                     {"fslp", FSLP},
                                                     {"hlld", HLLD},
                                                     {"fivewaves", FIVEWAVES},
    ...
    std::map<std::string, DivCleaning> div_cleaning_map{{"none", NO_DC}, {"dedner", DEDNER}, {"derigs", DERIGS}};
    div_cleaning = reader.GetMapValue(div_cleaning_map, "solvers", "div_cleaning", "dedner");
    ...
    // Gravity
    std::map<std::string, GravityMode> gravity_map{{"none", GRAV_NONE},
    ...
    std::map<std::string, AnalyticalGravityMode> analytical_gravity_map{{"hot_bubble", AGM_HOT_BUBBLE}};
...
{
  return {params.xmin + (i - params.ibeg + 0.5) * params.dx, params.ymin + (j - params.jbeg + 0.5) * params.dy};
}
...
Params readInifile(std::string filename)
{
  ...
  res.range_dom =
      ParallelRange({res.device_params.ibeg, res.device_params.jbeg}, {res.device_params.iend, res.device_params.jend});
  ...
  res.range_slopes = ParallelRange({res.device_params.ibeg - 1, res.device_params.jbeg - 1},
                                   {res.device_params.iend + 1, res.device_params.jend + 1});
...
void checkNegatives(const Array &Q, const Params &full_params)
{
  ...
  Kokkos::parallel_reduce(
      ...
      KOKKOS_LAMBDA(const int i,
                    const int j,
                    uint64_t &lnegative_density,
                    uint64_t &lnegative_pressure,
                    uint64_t &lnan_count) {
        if (Q(j, i, IR) < 0)
        {
          Q(j, i, IR) = epsilon;
