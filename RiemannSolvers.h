...
void hll(State &qL, State &qR, Flux& flux, real_t &pout, const DeviceParams &params) {
  ...
  if (SL >= 0.0) {
    ...
    flux.hydro = (SR*FL - SL*FR + SL*SR*(uR-uL)) / (SR-SL);
  } 
}

KOKKOS_INLINE_FUNCTION
void hllc(State &qL, State &qR, Flux &flux, real_t &pout, const DeviceParams &params) {
  ...
  flux.hydro = zero_state();
  flux.mhd = zero_state();
  flux.hydro[IR] = st[IR]*st[IU];
  flux.hydro[IU] = st[IR]*st[IU]*st[IU]+st[IP];
  flux.hydro[IV] = flux.hydro[IR]*st[IV];
  flux.hydro[IE] = (E + st[IP])*st[IU];
  // #ifdef MHD
  //   flux[IW] = 0.0;
  //   flux[IBX] = 0.0;
  //   flux[IBY] = 0.0;
...
KOKKOS_INLINE_FUNCTION
void hlld(State &qL, State &qR, Flux &flux, real_t &p_gas_out, const real_t Bx, const DeviceParams &params) {
  ...
  auto computeFlux = [&](const State &q, const real_t e_tot) -> Flux {
    Flux res{};

    res.hydro[IR]  = q[IR] * q[IU];
    res.hydro[IU]  = q[IR] * q[IU] * q[IU] + q[IP]; res.mhd[IU] = -q[IBX]*q[IBX];
    res.hydro[IV]  = q[IR] * q[IU] * q[IV];         res.mhd[IV] = - q[IBX]*q[IBY];
    res.hydro[IW]  = q[IR] * q[IU] * q[IW];         res.mhd[IW] = - q[IBX]*q[IBZ];
    res.mhd[IBX] = 0.0;
    res.mhd[IBY] = q[IBY]*q[IU] - q[IBX]*q[IV];
    res.mhd[IBZ] = q[IBZ]*q[IU] - q[IBX]*q[IW];     // Attention ! Dans e_tot il y a aussi du mag, à prendre en compte
    res.hydro[IE]  = (e_tot + q[IP]) * q[IU];       res.mhd[IE] = - q[IBX]*(q[IBX]*q[IU]+q[IBY]*q[IV]+q[IBZ]*q[IW]);
    res.mhd[IPSI] = 0.0;
    return res;
  };
...
KOKKOS_INLINE_FUNCTION
void FiveWaves(State &qL, State &qR, Flux &flux, real_t &pout, const DeviceParams &params) {
  ...
  //3. Commpute flux
  real_t uS = Ustar[IX]; // Vérifier s'il y a du mag là-dedans
  flux.hydro[IR]  = u[IR]  * uS;
  flux.hydro[IU]  = u[IU]  * uS; flux.mhd[IU] = Pstar[IX];
  flux.hydro[IV]  = u[IV]  * uS; flux.mhd[IV] = Pstar[IY];
  flux.hydro[IW]  = u[IW]  * uS; flux.mhd[IW] = Pstar[IZ];
  flux.hydro[IE]  = u[IE]  * uS; flux.mhd[IE] = Pstar[IX]*uS + Pstar[IY]*Ustar[IY] + Pstar[IZ]*Ustar[IZ];
  if (is_low_beta || (alfven_number > alfven_max)) {
    flux.mhd[IBX] = u[IBX] * uS - q[IBX] * Ustar[IX];
    flux.mhd[IBY] = u[IBY] * uS - q[IBX] * Ustar[IY];
    flux.mhd[IBZ] = u[IBZ] * uS - q[IBX] * Ustar[IZ];
    pout = Pstar[IX] - getMagneticPressure(Vect{q[IBX], q[IBY], q[IBZ]});
  }
  else {
    flux.mhd[IBX] = u[IBX] * uS - Bstar * Ustar[IX];
    flux.mhd[IBY] = u[IBY] * uS - Bstar * Ustar[IY];
    flux.mhd[IBZ] = u[IBZ] * uS - Bstar * Ustar[IZ];
...
// KOKKOS_INLINE_FUNCTION
// void FluxDerigs(State &qL, State &qR, Flux &flux, real_t ch, const DeviceParams &params){
...
// KOKKOS_INLINE_FUNCTION
// void FluxKEPEC(State &qL, State &qR, Flux &flux, real_t &pout, real_t ch, const DeviceParams &params){
...
KOKKOS_INLINE_FUNCTION
void IdealGLM(State &qL, State &qR, Flux &flux, real_t &pout, real_t ch, const DeviceParams &params){
